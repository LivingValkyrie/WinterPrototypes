...
public class Controller2D : RaycastController {
    ...
    /// </summary>
    /// <param name="velocity">The velocity of the controller.</param>
    /// <param name="input">the input from the object, used for players</param>
    /// <param name="standingOnPlatform">if set to <c>true</c> [standing on platform]. used to force collisions.below in cases 
    /// where a collision below the controller wouldn't be read.</param>
    ...
    void VerticalCollisions(ref Vector3 velocity) {
        ...
        for(int i = 0; i < verticalRayCount; i++) {
            ...
            if(hit) {
                //allow pass through of tagged obstacles when coming from below them
                if(hit.collider.tag == "Through") {
                    if(directionY == 1 || hit.distance == 0) {
                        continue;
                    }
                    //allow pass through of tagged obstacles when pressing down while above them.
    ...
    /// <summary>
    /// Resets the falling through platform variable of the CollisionInfo struct.
    /// </summary>
